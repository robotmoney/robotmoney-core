...
def test_the_rule_change_did_not_move_the_anchored_digest():
    ...
    to must be exactly what it was. This RE-DERIVES the digest from the golden
    file's bytes and compares it against the pinned constant, rather than trusting
    the constant to agree with itself — the #1280 discipline. A constant that
    could only ever agree with itself is the failure mode #1280 exists to prevent.
    """
    anchor = receipt.load_json(receipt.ANCHOR_DIGEST_PATH)
    pinned = {row["file"]: row for row in anchor["goldens"]}
    row = pinned["consensus-receipt.valid.canonical.txt"]

    assert receipt.canonicalize_receipt(VALID, SPEC) == GOLDEN
    payload = receipt.CANONICAL_PATH.read_bytes()
    assert len(payload) == row["byte_length"]
    # Guard the guard: a keccak256 that is quietly sha3-256 would agree with a
    # constant regenerated by the same wrong function.
    assert receipt.keccak256(b"") == (
        "0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470"
    )
    assert receipt.keccak256(payload) == row["keccak256"]
