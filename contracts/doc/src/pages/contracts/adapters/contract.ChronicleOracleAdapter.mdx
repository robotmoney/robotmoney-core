...
description: "BasketVault swap adapter that routes trades through Aerodrome Finance and prices NAV via a Chronicle on-chain push oracle."
---

# ChronicleOracleAdapter

[Git Source](https://github.com/robotmoney/robotmoney-core/blob/294ae939cc4db43ac7ec7bac05d27e475246f588/contracts/adapters/ChronicleOracleAdapter.sol)
...
<a id="constructor-address-address-bool-address-address-address"></a>
...
| _ | `uint24` | Venue-specific fee parameter (fee tier for Uniswap V3,<br/>ignored for Aerodrome which derives fee from pool config). |
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _ | `address` | Venue-specific pool address pairing `baseToken` with `quoteToken`. |
...
| _ | `uint32` | TWAP lookback window in seconds. |
