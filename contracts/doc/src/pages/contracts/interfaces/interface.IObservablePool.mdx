...
description: "Minimal pool surface required by the shared TWAP tick math: the two pool tokens, the Uniswap V3 `observe()` ABI, and the leading fields of `slot0()`. Both Aerodrome CL (Slipstream) pools and Uniswap V4 pools expose `token0`/`token1`/`observe()` identically to Uniswap V3, so the arithmetic-mean tick computation in `TwapTickMath` can be shared across adapters without depending on either concrete pool interface."
---

# IObservablePool

[Git Source](https://github.com/robotmoney/robotmoney-core/blob/294ae939cc4db43ac7ec7bac05d27e475246f588/contracts/interfaces/IObservablePool.sol)
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| secondsAgos | `uint32[]` | Array of elapsed seconds for each observation. `[window, 0]`<br/>is the canonical two-point TWAP read pattern. |
