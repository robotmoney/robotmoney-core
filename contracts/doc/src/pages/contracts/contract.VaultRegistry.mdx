---
title: "VaultRegistry"
description: "On-chain registry of authorised Robot Money vaults."
---
...
**Inherits:** [AdminFloorAccessControl](/contracts/lib/abstract.AdminFloorAccessControl)
...
<a id="registervault-address-vaultmetadata"></a>
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| vault | `address` | Address of the vault contract to register (must not be zero or<br/>already registered). |
| metadata | `VaultMetadata` | Human-readable name and asset address for the vault. |

<a id="retire-address"></a>
...
<a id="setvaultstatus-address-vaultstatus"></a>
...
<a id="setroutereligible-address-bool"></a>
...
<a id="migrateeligibility-address-bool-address-uint256"></a>
...
| defaultVaults | `address[]` | Ordered vault list for the new default vector; its<br/>length must equal the post-flip eligible count. |
| defaultBps | `uint256[]` | Parallel bps weights (must sum to BPS_DENOMINATOR). |

<a id="setrouter-address"></a>
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| eligible | `bool` | True iff governance has marked the vault as<br/>router-eligible. |

<a id="isvaultallexact-address"></a>
