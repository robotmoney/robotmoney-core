# UniswapV3AssetPositionAdapter
[Git Source](https://github.com/robotmoney/robotmoney-core/blob/cd218849ca46daf6891cc2b350fd6bac2d9f644b/contracts/adapters/UniswapV3AssetPositionAdapter.sol)
...
The first *inexact* `IPositionAdapter` (ADR-0010 §4): it custodies a
...
### POOL
The Uniswap V3 pool pairing `TOKEN` with `USDC` used for TWAP reads.
...
### SWAP_FEE
Uniswap fee tier for the execution pool (must resolve to `POOL`).
...
|`pool_`|`address`|       The Uniswap V3 pool pairing `token_`/`usdc_` (TWAP source).|
|`swapFee_`|`uint24`|    Uniswap fee tier of the execution pool (ORA-3: resolves to `pool_`).|
|`swapAdapter_`|`address`|Venue executor implementing `IBasketSwapAdapter`.|
...
Spot Uniswap V3 custody has no discrete claimable reward tokens — this
