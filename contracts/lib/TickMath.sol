...
/// @title TickMath
/// @notice Minimal port of Uniswap V3 `TickMath.getSqrtRatioAtTick`. The full
...
library TickMath {
    /// @notice The minimum tick that may be passed to `getSqrtRatioAtTick`.
    int24 internal constant MIN_TICK = -887272;
    /// @notice The maximum tick that may be passed to `getSqrtRatioAtTick`.
    int24 internal constant MAX_TICK = -MIN_TICK;

    /// @notice Raised when `tick` is outside the supported `[MIN_TICK, MAX_TICK]`
    ...
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        ...
        // Downcast to uint160 with rounding up: shift right 32 bits, rounding up
        // if any of the lower 32 bits are set. Equivalent to ceil(ratio / 2^32).
