...
//            docs/technical/unified-vault-spec.md §4.3 (adapter), §4.5 (M-S6);
...
/// @notice The Uniswap V4 sibling of `UniswapV3AssetPositionAdapter` (#1118):
...
contract UniswapV4AssetPositionAdapter is IPositionAdapter {
    using SafeERC20 for IERC20;
    using Math for uint256;

    // ─── Constants (verbatim from UniswapV3AssetPositionAdapter / BasketVault) ─
    ...
    /// @notice The Uniswap V4 pool pairing `TOKEN` with `USDC` used for TWAP reads
    ///         (`observe()`-compatible per EIP-7680, see `IUniswapV4Pool`).
    address public immutable POOL;
    /// @notice Uniswap V4 fee tier for the execution pool (must resolve to `POOL`).
    uint24 public immutable SWAP_FEE;
    /// @notice The V4 pool key's hook contract address. Constructor-asserted
    ...
    /// @dev M-S6: the V4 pool key's hooks address was non-zero at construction.
    ...
    /// @param pool_        The Uniswap V4 pool pairing `token_`/`usdc_` (TWAP source).
    /// @param swapFee_     Uniswap V4 fee tier of the execution pool (ORA-3: resolves to `pool_`).
    /// @param hooks_       The V4 pool key's hook contract address. MUST be
    ///                     `address(0)` — hooked pools are out of scope/unsafe
    ///                     for this integration (M-S6) and revert construction.
    /// @param swapAdapter_ Venue executor implementing `IBasketSwapAdapter`
    ///                     (concretely `UniswapV4SwapAdapter`, which itself only
    ///                     ever trades hookless pools).
    /// @dev Asserts `hooks_ == address(0)` FIRST (cheapest check, M-S6), then the
    ...
    constructor(
        ...
        // M-S6 / spec §4.5: close the read-only-reentrancy hook-callback edge by
        // refusing to construct against a hooked pool at all.
        if (hooks_ != address(0)) revert HookedPoolNotSupported(hooks_);

        // ORA-3 / F-09: the pool the NAV TWAP is read from must be the pool trades
        // execute against (fee tier for Uniswap V4).
        BasketAssetConfigGuard.requireExecutionPoolMatchesTwap(
            pool_, swapFee_, BasketAssetConfigGuard.Venue.V4
        );
        ...
        returns (uint256 valueAdded)
    {
        ...
        // The V4 hook-callback edge is additionally closed at construction (M-S6):
        // `HOOKS == address(0)` is asserted immutably, so there is no hook to
        // re-enter through even if the venue routed via a hookful PoolManager.
        ...
        returns (uint256 usdcOut)
    {
        ...
        // ERC777 tokens. Same M-S6 hook-callback closure as `deploy` above.
    ...
    /// @inheritdoc IPositionAdapter
    /// @dev Spot Uniswap V4 custody has no discrete claimable reward tokens — this
