...
/// @notice The Aerodrome (Slipstream concentrated-liquidity) counterpart of
...
///         token.
contract AerodromeAssetPositionAdapter is IPositionAdapter {
    using SafeERC20 for IERC20;
    using Math for uint256;

    // ─── Constants (verbatim from UniswapV3AssetPositionAdapter, issue #1118) ──
    ...
    /// @notice The Aerodrome Slipstream CL pool pairing `TOKEN` with `USDC`,
    ///         used for TWAP reads via its Uniswap-V3-shaped `observe()` ABI.
    address public immutable POOL;
    /// @notice Aerodrome Slipstream tick spacing of the execution pool (must
    ...
    /// @param pool_        The Aerodrome Slipstream CL pool pairing `token_`/`usdc_`
    ///                     (TWAP source).
    /// @param tickSpacing_ Aerodrome Slipstream tick spacing of the execution pool
    ///                     (ORA-3: resolves to `pool_` via the CL factory).
    ...
    constructor(
        ...
        // execute against (tick spacing for Aerodrome Slipstream).
        BasketAssetConfigGuard.requireExecutionPoolMatchesTwap(
            pool_, tickSpacing_, BasketAssetConfigGuard.Venue.Aerodrome
        );
        ...
        // Aerodrome Slipstream CL pools expose the identical `slot0`/`observe`/
        // `liquidity` ABI as Uniswap V3, so the shared guard applies unchanged.
        ...
        returns (uint256 valueAdded)
    {
        ...
        SWAP_ADAPTER.swap(
            USDC, TOKEN, TICK_SPACING, usdcIn, minTokenOut, address(this), block.timestamp
        );
        ...
        returns (uint256 usdcOut)
    {
        ...
        usdcOut = SWAP_ADAPTER.swap(
            TOKEN, USDC, TICK_SPACING, tokenToSell, internalFloor, VAULT, block.timestamp
        );
    ...
    /// @inheritdoc IPositionAdapter
    /// @dev Spot Aerodrome custody has no discrete claimable reward tokens — this
