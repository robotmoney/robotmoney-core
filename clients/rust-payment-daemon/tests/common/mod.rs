//! Shared test fixtures for the `rmpd` CLI integration tests.
//!
...
use mockito::Matcher;
use rust_payment_daemon::gateway::{MockUsdc, RobotMoneyGateway};
...
impl Fixture {
    ...
    pub fn build(rpc_url: &str, chain_id: u64) -> Self {
        ...
        let config_path = tmp.path().join("rmpd.toml");
        let toml = format!(
            r#"chain_id              = {chain_id}
rpc_url               = "{rpc_url}"
