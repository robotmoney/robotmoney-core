...
pub const PASSPHRASE_ENV_VAR: &str = "RMPD_KEYSTORE_PASSPHRASE";
...
pub struct Keystore {
    ...
    pub version: u32,
    /// Discriminator string. Must be `"rmpd-software-keystore"`.
    pub kind: String,
...
impl SoftwareSigner {
    ...
    /// `passphrase`. Used by `rmpd keystore new` and by tests.
    ///
    ...
    pub fn create_keystore<P: AsRef<Path>>(
        ...
        let keystore = Keystore {
            version: 1,
            kind: "rmpd-software-keystore".to_string(),
    ...
    pub fn load<P: AsRef<Path>>(
        ...
        eprintln!(
            "rmpd[WARN] software signer enabled: keystore decrypted in process memory \
    ...
    pub fn from_keystore(keystore: &Keystore, passphrase: &[u8]) -> Result<Self, SignerError> {
        ...
        if keystore.kind != "rmpd-software-keystore" {
            return Err(SignerError::ErrKeystoreFormat(format!(
