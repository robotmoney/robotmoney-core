...
//! is single-flight: each `rmpd deposit` invocation acquires an
...
//! [`RmpdError::ErrConcurrentInvocation`] — there is no waiting, no
...
impl AgentLock {
    ...
    ///
    /// `state_dir` is created if missing — `rmpd` is normally run by an
    ...
    ///
    /// On contention returns [`RmpdError::ErrConcurrentInvocation`].
    pub fn acquire(state_dir: &Path, address: &Address) -> Result<Self> {
        std::fs::create_dir_all(state_dir).map_err(|e| {
            RmpdError::ErrConfig(format!(
...
mod tests {
    ...
    fn second_acquire_while_held_returns_concurrent_invocation() {
        ...
        assert!(
            matches!(err, RmpdError::ErrConcurrentInvocation),
    ...
    fn racing_threads_only_one_winner() {
        ...
        let errs = [&r1, &r2]
            .iter()
            .filter(|r| matches!(r, Err(RmpdError::ErrConcurrentInvocation)))
            .count();
