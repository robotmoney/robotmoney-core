//! `rmpd self-check` — read-only backend report (v0 §9.2 + preflight snapshot).
//!
...
pub struct SelfCheckOutput {
    ...
    pub ok: bool,
    /// Variant name of the [`RmpdError`] that caused the refusal, when
...
pub fn run(config_path: &Path, pretty: bool) -> i32 {
    let cfg = match Config::from_path(config_path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("rmpd self-check: failed to load config: {e}");
    ...
    let passphrase = match std::env::var(PASSPHRASE_ENV_VAR) {
        ...
        Err(_) => {
            eprintln!(
                "rmpd self-check: ${PASSPHRASE_ENV_VAR} is unset; refusing to prompt on stdin from a non-interactive command"
            );
            return EXIT_STARTUP_FAIL;
        }
    };
    let signer = match SoftwareSigner::load_with_passphrase(
        ...
        Ok(s) => s,
        Err(e) => {
            eprintln!("rmpd self-check: signer load failed: {e}");
            return EXIT_STARTUP_FAIL;
        }
    };
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("rmpd self-check: tokio runtime build failed: {e}");
            return EXIT_STARTUP_FAIL;
        }
    };
    ...
    let rpc = match RpcClient::new(&cfg.rpc_url) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("rmpd self-check: rpc client init failed: {e}");
            return EXIT_STARTUP_FAIL;
        }
    };
...
mod tests {
    ...
    fn error_name_covers_every_preflight_refusal() {
        ...
        assert_eq!(
            error_name(&RmpdError::ErrChainIdMismatch),
            "ErrChainIdMismatch"
        );
        assert_eq!(
            error_name(&RmpdError::ErrCodeHashMismatch),
            "ErrCodeHashMismatch"
        );
        assert_eq!(error_name(&RmpdError::ErrGatewayPaused), "ErrGatewayPaused");
        assert_eq!(
            error_name(&RmpdError::ErrAgentNotAuthorized),
            "ErrAgentNotAuthorized"
        );
        assert_eq!(
            error_name(&RmpdError::ErrAllowanceInsufficient),
            "ErrAllowanceInsufficient"
        );
        assert_eq!(
            error_name(&RmpdError::ErrBalanceInsufficient),
