...
//! [`RmpdError::ErrRpcServer`]; transport failures map to
//! [`RmpdError::ErrRpcTransport`]; deserialisation issues to
//! [`RmpdError::ErrRpcDecode`].
...
impl RpcClient {
    ...
    pub fn new(url: impl Into<String>) -> Result<Self> {
        ...
        reqwest::Url::parse(&url).map_err(|e| RmpdError::ErrConfig(format!("rpc url: {e}")))?;
        let http = reqwest::Client::builder()
            .timeout(DEFAULT_TIMEOUT)
            .build()
            .map_err(|e| RmpdError::ErrRpcTransport(e.to_string()))?;
        Ok(Self {
            url,
            http,
    ...
    pub async fn call<T: DeserializeOwned>(&self, method: &str, params: Value) -> Result<T> {
        ...
        if !status.is_success() {
            return Err(RmpdError::ErrRpcTransport(format!(
                "HTTP {status}: {}",
        ...
        let parsed: JsonRpcResponse = serde_json::from_slice(&body)
            .map_err(|e| RmpdError::ErrRpcDecode(format!("response not JSON: {e}")))?;

        if let Some(err) = parsed.error {
            return Err(RmpdError::ErrRpcServer {
                code: err.code,
                message: err.message,
            });
        }
        ...
        serde_json::from_value(result)
            .map_err(|e| RmpdError::ErrRpcDecode(format!("result decode: {e}")))
    }
...
mod tests {
    ...
    fn new_rejects_bad_url() {
        ...
        assert!(matches!(err, RmpdError::ErrConfig(_)), "got {err:?}");
    }
    ...
    async fn server_error_object_maps_to_err_rpc_server() {
        ...
        match err {
            RmpdError::ErrRpcServer { code, message } => {
    ...
    async fn http_500_maps_to_err_rpc_transport() {
        ...
        let client = RpcClient::new(server.url()).unwrap();
        let err = client.chain_id().await.unwrap_err();
        assert!(matches!(err, RmpdError::ErrRpcTransport(_)), "got {err:?}");
    }
    ...
    async fn malformed_body_maps_to_err_rpc_decode() {
        ...
        let client = RpcClient::new(server.url()).unwrap();
        let err = client.chain_id().await.unwrap_err();
        assert!(matches!(err, RmpdError::ErrRpcDecode(_)), "got {err:?}");
    }
