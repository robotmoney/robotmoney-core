...
/// `newestBlock`.
///
/// Returns [`RmpdError::ErrFeeCapExceeded`] when `2 * baseFeeNext +
/// priorityFee > cap_wei`.
...
fn base_fee_for_next_block(fh: &FeeHistory) -> Result<u128> {
    fh.base_fee_per_gas
        .last()
        .copied()
        .ok_or_else(|| RmpdError::ErrRpcDecode("eth_feeHistory: empty base_fee_per_gas".into()))
}
...
mod tests {
    ...
    fn cap_exceeded_refuses_with_named_error() {
        ...
        assert!(matches!(err, RmpdError::ErrFeeCapExceeded), "got {err:?}");
    }
