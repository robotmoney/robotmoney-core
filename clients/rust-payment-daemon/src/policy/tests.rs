...
async fn chain_id_mismatch_refuses() {
    ...
    assert!(matches!(err, RmpdError::ErrChainIdMismatch), "got {err:?}");
}
...
async fn empty_code_refuses_with_code_hash_mismatch() {
    ...
    let pf = Preflight::new(&rpc, &cfg);
    let err = pf.run(inputs(100)).await.unwrap_err();
    assert!(matches!(err, RmpdError::ErrCodeHashMismatch), "got {err:?}");
}
...
async fn paused_gateway_refuses() {
    ...
    let pf = Preflight::new(&rpc, &cfg);
    let err = pf.run(inputs(100)).await.unwrap_err();
    assert!(matches!(err, RmpdError::ErrGatewayPaused), "got {err:?}");
}
...
async fn usdc_address_mismatch_refuses() {
    ...
    let err = pf.run(inputs(100)).await.unwrap_err();
    match err {
        RmpdError::ErrConfig(msg) => assert!(msg.contains("usdc"), "msg = {msg}"),
...
async fn vault_address_mismatch_refuses() {
    ...
    let err = pf.run(inputs(100)).await.unwrap_err();
    match err {
        RmpdError::ErrConfig(msg) => assert!(msg.contains("vault"), "msg = {msg}"),
...
async fn inactive_agent_refuses() {
    ...
    let err = pf.run(inputs(100)).await.unwrap_err();
    assert!(
        matches!(err, RmpdError::ErrAgentNotAuthorized),
        "got {err:?}"
    );
}
...
async fn over_per_payment_cap_refuses() {
    ...
    match err {
        RmpdError::ErrConfig(m) => assert!(m.contains("maxPerPayment"), "m = {m}"),
...
async fn over_window_cap_refuses() {
    ...
    let err = pf.run(inputs(1_000)).await.unwrap_err();
    match err {
        RmpdError::ErrConfig(m) => assert!(m.contains("maxPerWindow"), "m = {m}"),
...
async fn allowance_too_low_refuses() {
    ...
    let err = pf.run(inputs(100)).await.unwrap_err();
    assert!(
        matches!(err, RmpdError::ErrAllowanceInsufficient),
        "got {err:?}"
    );
}
...
async fn balance_too_low_refuses() {
    ...
    let err = pf.run(inputs(100)).await.unwrap_err();
    assert!(
        matches!(err, RmpdError::ErrBalanceInsufficient),
        "got {err:?}"
    );
}
