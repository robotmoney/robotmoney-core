...
description: >
  Onboard an operator's agent onto the Robot Money Swarm — use when the operator
  says "onboard my swarm agent", "join the Robot Money swarm", "set up my swarm
  member", or the older phrasings "onboard my committee agent" / "join the Robot
  Money committee". Walks through installing the
...
You are setting up **this machine** as a Robot Money **Swarm** member on behalf
of its human owner. The swarm meets in sessions; each
