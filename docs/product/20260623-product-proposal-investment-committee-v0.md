...
> §2.4, §4.8, §5.1, §5.4, §7.4 — see §3 for shipped artifacts). The remaining
...
> (§2.1, §3.1), which is scoped as v0.1 and requires its own payload schema,
> gateway surface, indexer tables, `rmpc` command, and off-chain worker
> (see §4 and the consolidated adversarial review `20260825-adversarial-review-ic-v0.md`).
...
|---|---|---|---|
| Membership | **Admin-gated, timelock-held, capped.** `ADMIN_ROLE` on the IC contract is held by the `TimelockController` (`docs/architecture.md` §4.5, §4.8) with proposer/canceller on the Safe (`0x88bA…75A0`). V0 allowlist is capped at **3–5 internal seats** (Athena / Robot Money / Woon — see §6.1). Membership is registry state (`VaultRegistry.isRouterEligible` analogue, `docs/architecture.md` §7.3), not a code variant. Public `AgentRegistered`/`AgentRevoked` events are the auditable membership log. No permissionless onboarding in v0. | Shipped (cap is a product decision) | `contracts/gateway/InvestmentCommitteePolicy.sol:37,164-177`, `docs/architecture.md:134-136,4.5` |
...
the prior one-liner at `v0.md:54` is superseded by this section and by the
consolidated review `20260825-adversarial-review-ic-v0.md` §4.

- **Identity:** same as votes — EOA-via-gateway, `onlyGateway` on the receipt
  contract, `COMMITTEE_AGENT_ROLE` check. No parallel `AGENT_ROLE` on the
  receipt contract (cf. stashed `ConsensusRebalanceReceipt.sol:28-29` — to be
  removed; see `20260825-adversarial-review-ic-v0.md` §4a).
...
- **Payload:** TBD — fixed-shape JSON schema committed to the repo (like
  ...
  v0.1 does not ship until the schema is committed (see §6.2).
...
### Shipped (Investment Committee — `issue #1044`)
...
### Extend for v0.1 (Consensus rebalance receipts — proposed)
...
|---|---|---|---|
| 1 | Exact split of fields on-chain vs off-chain memo | **Shipped for votes; TBD for receipts.** Votes: on-chain stores the commitment tuple — `rationale_uri`, `vote_digest` (`voteJsonHash`), `prompt_hash`, `inputs_digest`, `timestamp`, `schema_version` plus structured `agent`, `vault`, `stance`, `target_weight_bps`, `confidence` — per `tests/fixtures/committee-vote.schema.json:1-60` and `InvestmentCommitteePolicy.sol:71-97,197-246`. Off-chain memo at `rationale_uri` carries the full narrative rationale, bound by the digest. Receipts: field split is TBD pending the receipt JSON schema (§2.1, §6.2) — no receipt schema is committed on this branch. | Contract + schema |
...
- **Receipt JSON schema — owner: product + contract.** What fields are in the
  ...
  **Default:** do not build receipts; keep v0.1 unscoped until a schema is
  ...
  and `contract/src/signing.js:5-20`).

- **Release quorum & policy — owner: product.** Is `consensusReleaseReceipt`
...
- **Genesis seat attestation for external orgs — owner: ops + product.**
  Beyond the 3–5 internal genesis seats (§4.3), what is the onboarding bar
