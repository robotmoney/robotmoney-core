...
| Router eligibility | `contracts/VaultRegistry.sol` (`setRouterEligible:375`, `routerEligibleCount:130`, `VaultStatus.Paused:73`), ADR-0002 | **reusable**, with an interlock | Adding a B20 vault as a router leg is the existing `defaultWeights`-length interlock, not new code. Exclusion is available but **manual**: `setRouterEligible` is `ADMIN_ROLE`-gated and reversible in both directions (`:375`), and a transient, reversible `VaultStatus.Paused` exists (`:73`, `:331`) — but flipping eligibility is itself gated by the interlock, which reverts `StaleDefaultWeightsLength` unless the router's default weight vector is re-set to span the new eligible set (`:392-397`). What does **not** exist is any *automatic* exclusion of a leg whose oracle has gone stale, so a weekly 24/5 gap would need a standing operational response rather than a state the registry reaches on its own. `UNVERIFIED` whether `PortfolioRouter` degrades or reverts when one leg's `totalAssets()` reverts. |
