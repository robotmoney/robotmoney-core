...
- **Additional review trigger.** Enabling the browser-generated flow in any released build requires a named review on a follow-up issue with the label `security-review` and explicit sign-off from a human admin role-holder. The downstream implementation issue (#60) carries this gate in its acceptance criteria; without that review the dapp ships with the flow disabled by feature flag (default off). The security-review record is `docs/technical/dapp-browser-keygen-review.md` (issue #84); its §6 fork-vs-mainnet gate and §7 go/no-go conditions are the mechanical preconditions for any flag flip.
