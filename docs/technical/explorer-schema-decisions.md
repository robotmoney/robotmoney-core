...
- **Decision.** The indexer never ingests blocks within `CONFIRMATIONS = 5` of `eth_blockNumber`. On every tick, the indexer fetches `eth_getBlockByNumber(last_indexed_block, false)` and compares its hash to the stored `blocks.hash` for that height. If they differ, a reorg is detected:
  ...
  - In a single transaction, `DELETE … WHERE chain_id = $1 AND block_number > reorg_root` from **every block-scoped table**. Which tables those are is *derived from the live schema at rollback time*, not listed here or in the code: a base table qualifies when it carries a `block_number` column, and the deletion order is taken from the live foreign-key graph so a child (e.g. `governance_votes`) is always deleted before the parent it references (`governance_proposals`). See `Db::delete_above_block` and `db::REORG_ROLLBACK_EXCLUSIONS` in `services/explorer-indexer/src/db.rs`.
    - **Schema contract for new tables (issue #1283).** A new table whose rows belong to a block MUST carry both `chain_id` and `block_number`; it is then rolled back by the migration that creates it, with no code change. A table with `block_number` but no `chain_id` is a hard error at rollback time, not a silent skip. The only exemption is an entry in `REORG_ROLLBACK_EXCLUSIONS` stating why those rows legitimately survive an orphaned block. This replaces the previous hand-maintained list, which migration 0014 outgrew: `committee_votes` and `regime_snapshots` were block-scoped but absent from it, so a reorg past a `VoteSubmitted` left the committee and regime-feed endpoints serving orphaned rows as current state.
    - Rows whose block-scope is an *in-place mutation* rather than a row identity are reconciled separately in the same transaction: `vaults.status` (IDX-8) is re-derived from surviving `vault_status_events`, `consensus_receipts.released` (§4.9) is un-released when the release block was orphaned, and `indexer_runs.last_indexed_block` (IDX-2) is capped down to `reorg_root`.
