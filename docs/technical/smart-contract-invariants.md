...
> ✅ HOLDS (fixed #971, NC-11) · `RobotMoneyVault._withdraw` now derives both the exit fee and the user payout from the amount `_pullProportional` ACTUALLY realises (idle applied + USDC genuinely pulled, clamped to the share-implied gross), not the share-implied gross itself; an over-reporting adapter that under-delivers reverts (`InsufficientAdapterLiquidity`) rather than silently covering the shortfall from other holders' idle USDC · fuzz (over-reporting-adapter handler) — `FvInvariants.t.sol::test_FEE2_expectedFail_feeChargedOnRealizedProceeds`; deep proofs `RobotMoneyVault.t.sol::test_FEE2_exitFeeOnRealizedProceeds` / `test_FEE2_overReportingAdapterCannotSocializeLoss`.
