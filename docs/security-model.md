...
| Malicious strategy adapter onboarding | Mitigated | `RobotMoneyVault.addAdapter` now requires Safe-governed approval of the exact adapter address and runtime `codehash`, plus adapter `USDC()` / `VAULT()` compatibility checks. Deposit, `rebalance`, and `adminRebalance` re-check address eligibility before transferring USDC; emergency withdrawal and force removal remain usable for already-active adapters after approval is revoked. |
