...
| Attack | Status | Notes / evidence |
|---|---|---|
| ERC-4626 inflation / first-depositor share-price attack | Unaddressed | The vault uses OpenZeppelin's ERC-4626; mitigation depends on whether the OZ "virtual offset" decimals adjustment is enabled. Triage: confirm `_decimalsOffset()` value in `RobotMoneyVault.sol`; document expected first-deposit conditions (admin seed, non-zero floor) |
